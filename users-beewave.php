...
if (($select_vendor_table == true) && ($select_vendor_table["website_url"] == $_SERVER["HTTP_HOST"]) && ($select_vendor_table["status"] == 1)) {
	...
	if ($virtual_accounttype == "static") {
		...
		if (mysqli_num_rows($check_if_banks_exists) == 1) {
			...
			$beewave_verify_transaction = json_decode(confirmPaymentDeposited("GET", "https://merchant.beewave.ng/api/v1/collection/verify?access_key=" . $beewave_keys["secret_key"] . "&&transaction_ref=" . $transaction_id, ["Accept: application/json", "Content-Type: application/json"], ""), true);
