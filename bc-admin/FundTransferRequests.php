	...
	<?php include("../func/bc-admin-header.php"); ?>
        ...
        <div class="bg-3 m-width-100 s-width-100 m-height-auto s-height-auto m-margin-bm-2 s-margin-bm-2">
            <div class="bg-3 m-width-96 s-width-96 m-height-auto s-height-auto m-margin-tp-5 s-margin-tp-2 m-margin-lt-2 s-margin-lt-2">
                <span style="user-select: auto;" class="text-bg-1 color-4 text-bold-600 m-font-size-20 s-font-size-25 m-inline-block-dp s-inline-block-dp m-margin-bm-1 s-margin-bm-1">FUND TRANSFER REQUEST</span><br>
                <form method="get" action="FundTransferRequests.php" class="m-margin-tp-1 s-margin-tp-1">
                    <input style="user-select: auto;" name="searchq" type="text" value="<?php echo trim(strip_tags($_GET["searchq"])); ?>" placeholder="Reference No, Username, amount e.t.c" class="input-box color-4 bg-2 outline-none br-radius-5px br-width-4 br-color-4 m-width-40 s-width-30 m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" />
            ...
            <div style="border: 1px solid var(--color-4); user-select: auto;" class="bg-3 m-width-95 s-width-95 m-height-auto s-height-auto m-scroll-x s-scroll-x m-margin-lt-2 s-margin-lt-2">
                <table class="table-tag m-font-size-12 s-font-size-14" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                    <tr>
                    	<th>S/N</th><th>Reference</th><th>Username ID</th><th>Receiver</th><th style="">Description</th><th>Amount</th><th>Amount Paid</th><th>Mode</th><th>Status</th><th>Date</th><th>Action</th>
                    </tr>
                    <?php
                    if(mysqli_num_rows($get_user_pending_transaction_details) >= 1){
                    	while($user_transaction = mysqli_fetch_assoc($get_user_pending_transaction_details)){
                    		...
                    		$countTransaction += 1;
                    		$reject_without_refund_payment_order = '<span onclick="adminFundTransferRequestStatus(`3`,`'.$user_transaction["reference"].'`,`'.$user_transaction["username"].'`);" style="text-decoration: underline; color: orange;" class="a-cursor">Reject (No Refund) Transfer</span>';
                    		$reject_payment_order = '<span onclick="adminFundTransferRequestStatus(`1`,`'.$user_transaction["reference"].'`,`'.$user_transaction["username"].'`);" style="text-decoration: underline; color: red;" class="a-cursor">Reject Transfer</span>';
                    		$accept_payment_order = '<span onclick="adminFundTransferRequestStatus(`2`,`'.$user_transaction["reference"].'`,`'.$user_transaction["username"].'`);" style="text-decoration: underline; color: green;" class="a-cursor">Accept Transfer</span>';
                    		$all_payment_order_action =$reject_without_refund_payment_order." | ".$reject_payment_order." | ".$accept_payment_order;
                    		echo 
                    		'<tr>
                    			<td>'.$countTransaction.'</td><td style="user-select: auto;">'.$user_transaction["reference"].'</td><td>'.ucwords($user_transaction["username"]).'</td><td>'.ucwords($user_transaction["recipient_username"]).'</td><td>'.$user_transaction["description"].'</td><td>'.toDecimal($user_transaction["amount"], 2).'</td><td>'.toDecimal($user_transaction["discounted_amount"], 2).'</td><td>'.$user_transaction["mode"].'</td><td>'.tranStatus($user_transaction["status"]).'</td><td>'.formDate($user_transaction["date"]).'</td><td>'.$all_payment_order_action.'</td>
                    		</tr>';
                    	}
                    }
                    ?>
                ...
                <table class="table-tag m-font-size-12 s-font-size-14" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                    ...
                    if(mysqli_num_rows($get_user_failed_transaction_details) >= 1){
                    	while($user_transaction = mysqli_fetch_assoc($get_user_failed_transaction_details)){
                    		$transaction_type = ucwords($user_transaction["type_alternative"]);
                    		$countTransaction += 1;
                    		$all_payment_order_action = "Rejected";
                    		echo 
                    		'<tr>
                    			<td>'.$countTransaction.'</td><td style="user-select: auto;">'.$user_transaction["reference"].'</td><td>'.ucwords($user_transaction["username"]).'</td><td>'.ucwords($user_transaction["recipient_username"]).'</td><td>'.$user_transaction["description"].'</td><td>'.toDecimal($user_transaction["amount"], 2).'</td><td>'.toDecimal($user_transaction["discounted_amount"], 2).'</td><td>'.$user_transaction["mode"].'</td><td>'.tranStatus($user_transaction["status"]).'</td><td>'.formDate($user_transaction["date"]).'</td><td>'.$all_payment_order_action.'</td>
                    		</tr>';
