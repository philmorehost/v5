    ...
    if(isset($_POST["login"])){
    	...
    	if(!empty($email) && !empty($pass)){
			...
			if(mysqli_num_rows($get_admin_details) == 1){
				$md5_pass = md5($pass);
				$check_admin_password_details = mysqli_query($connection_server, "SELECT * FROM sas_vendors WHERE id='".$get_vendor_details["id"]."' && email='$email' && password='$md5_pass'");
...
<!DOCTYPE html>
<head>
    <title>Login | <?php echo $get_all_super_admin_site_details["site_title"]; ?></title>
    <meta charset="UTF-8" />
...
<body>	
    <div style="text-align: center;" class="bg-10 m-block-dp s-block-dp m-position-abs s-position-abs br-radius-5px m-width-94 s-width-50 m-height-auto s-height-auto m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-padding-tp-25 s-padding-tp-3 m-padding-bm-25 s-padding-bm-3 m-margin-tp-20 s-margin-tp-10 m-margin-lt-2 s-margin-lt-24">
        ...
        <span style="user-select: auto;" class="text-bg-1 color-4 m-inline-block-dp s-inline-block-dp text-bold-500 m-font-size-20 s-font-size-25 m-margin-bm-2 s-margin-bm-2">ADMIN LOGIN</span><br>
        <form method="post" action="">
            <input style="text-align: center; text-transform: lowercase;" name="email" type="email" value="" placeholder="Email" class="input-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-60 s-width-45 m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" autocomplete="on" required/><br/>
		...
		<span style="user-select: auto;" class="color-4 m-block-dp s-block-dp m-font-size-16 s-font-size-18">
			<a href="<?php echo $web_http_host; ?>/bc-admin/PasswordRecovery.php">
