<?php session_start();
    ...
    $array_mtn_product_variety = array("500mb","1gb","1.5gb","2gb","3gb","5gb","10gb");
    $array_airtel_product_variety = array("500mb","1gb","2gb","5gb");
    $array_glo_product_variety = array("500mb","1gb");
    $array_9mobile_product_variety = array();
            ...
            if(!empty($product_name)){
                if(in_array($product_name, $products_array)){
                    if(is_numeric($item_status) && in_array($item_status, array("0", "1"))){
                        $select_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='$api_id' && api_type='datacard'");
                        $select_datacard_status_lists = mysqli_query($connection_server, "SELECT * FROM sas_datacard_status WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
                        if(mysqli_num_rows($select_api_lists) == 1){    
                            if(mysqli_num_rows($select_datacard_status_lists) == 0){
                                mysqli_query($connection_server, "INSERT INTO sas_datacard_status (vendor_id, api_id, product_name, status) VALUES ('".$get_logged_admin_details["id"]."', '$api_id', '$product_name', '$item_status')");
                            }else{
                                mysqli_query($connection_server, "UPDATE sas_datacard_status SET api_id='$api_id', status='$item_status' WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
                            }
                            ...
                            foreach($account_level_table_name_arrays as $account_level_table_name){ 
                                ...
                                if(mysqli_num_rows($select_product_details) == 1){
                                    ...
                                    $select_all_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_type='datacard'");
                        ...
                        //Invalid DATACARD Status
                        $json_response_array = array("desc" => "Invalid datacard Status");
                        $json_response_encode = json_encode($json_response_array,true);
                    }
                }else{
...
<body>
    <?php include("../func/bc-admin-header.php"); ?>    
        <div style="text-align: center;" class="bg-10 m-block-dp s-block-dp m-position-rel s-position-rel br-radius-5px m-width-94 s-width-94 m-height-auto s-height-auto m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-padding-tp-5 s-padding-tp-3 m-padding-bm-5 s-padding-bm-3 m-margin-lt-2 s-margin-lt-2 m-margin-bm-2 s-margin-bm-2">
            <span style="user-select: auto;" class="text-bg-1 color-4 text-bold-500 m-font-size-25 s-font-size-30 m-inline-block-dp s-inline-block-dp m-margin-bm-1 s-margin-bm-1">DATACARD SETTINGS</span><br>
            ...
            <div style="text-align: center; user-select: auto;" class="bg-3 m-inline-block-dp s-inline-block-dp m-width-70 s-width-50 m-margin-tp-1 s-margin-tp-1 m-margin-bm-0 s-margin-bm-0">
                <img alt="Airtel" id="airtel-lg" product-name-array="mtn,airtel,glo,9mobile" src="/asset/airtel.png" onclick="tickProduct(this, 'airtel', 'api-product-name', 'install-product', 'png');" class="a-cursor bg-2 br-radius-100px m-inline-block-dp s-inline-block-dp onhover-img-filter m-width-20 s-width-22 m-margin-tp-1 s-margin-tp-1 m-margin-bm-1 s-margin-bm-1 "/>
                <img alt="MTN" id="mtn-lg" product-name-array="mtn,airtel,glo,9mobile" src="/asset/mtn.png" onclick="tickProduct(this, 'mtn', 'api-product-name', 'install-product', 'png');" class="a-cursor bg-2 br-radius-100px m-inline-block-dp s-inline-block-dp onhover-img-filter m-width-20 s-width-22 m-margin-tp-1 s-margin-tp-1 m-margin-bm-1 s-margin-bm-1 m-margin-lt-1 s-margin-lt-1"/>
                <img alt="Glo" id="glo-lg" product-name-array="mtn,airtel,glo,9mobile" src="/asset/glo.png" onclick="tickProduct(this, 'glo', 'api-product-name', 'install-product', 'png');" class="a-cursor bg-2 br-radius-100px m-inline-block-dp s-inline-block-dp onhover-img-filter m-width-20 s-width-22 m-margin-tp-1 s-margin-tp-1 m-margin-bm-1 s-margin-bm-1 m-margin-lt-1 s-margin-lt-1"/>
                <img alt="9mobile" id="9mobile-lg" product-name-array="mtn,airtel,glo,9mobile" src="/asset/9mobile.png" onclick="tickProduct(this, '9mobile', 'api-product-name', 'install-product', 'png');" class="a-cursor bg-2 br-radius-100px m-inline-block-dp s-inline-block-dp onhover-img-filter m-width-20 s-width-22 m-margin-tp-1 s-margin-tp-1 m-margin-bm-1 s-margin-bm-1 m-margin-lt-1 s-margin-lt-1"/>
            </div><br/>
            <form method="post" action="">
                ...
                <div style="text-align: center;" class="color-4 bg-3 m-inline-block-dp s-inline-block-dp m-font-size-14 s-font-size-16 m-width-60 s-width-45">
                    <span id="user-status-span" class="a-cursor" style="user-select: auto;">DATACARD STATUS</span>
                </div><br/>
                <select style="text-align: center;" id="" name="item-status" onchange="" class="select-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-63 s-width-47 m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" required/>
                    <option value="" default hidden selected>Choose DATACARD Status</option>
                    <option value="1" >Enabled</option>
                    <option value="0" >Disabled</option>
                </select><br/>
            ...
            <span style="user-select: auto;" class="color-4 text-bold-600 m-font-size-18 s-font-size-20 m-inline-block-dp s-inline-block-dp m-margin-tp-2 s-margin-tp-2 m-margin-bm-1 s-margin-bm-1">INSTALLED DATACARD STATUS</span><br>
                    ...
                    <?php
                        ...
                        if(mysqli_num_rows($select_item_lists) >= 1){
                            while($list_details = mysqli_fetch_assoc($select_item_lists)){
                                $select_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='".$list_details["api_id"]."' && api_type='datacard'");
                                if(mysqli_num_rows($select_api_lists) == 1){
            ...
            <span style="user-select: auto;" class="color-4 text-bold-600 m-font-size-18 s-font-size-20 m-inline-block-dp s-inline-block-dp m-margin-tp-2 s-margin-tp-2 m-margin-bm-1 s-margin-bm-1">DATACARD DISCOUNT</span><br>
            <div style="border: 1px solid var(--color-4); user-select: auto; cursor: grab;" class="bg-3 m-inline-block-dp s-inline-block-dp m-width-90 s-width-47 m-height-auto s-height-auto m-scroll-x s-scroll-x m-margin-lt-0 s-margin-lt-0">
                <form method="post" action="">
                    <table style="width: 100% !important;" class="table-tag m-font-size-12 s-font-size-14" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                        <tr>
                        ...
                        <?php
                            ...
                            foreach($item_name_array_2 as $products){
                                ...
                                $get_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='".$get_item_status_details["api_id"]."' && api_type='datacard'");
                                        ...
                                        echo 
                                            '<tr style="background-color: transparent !important;">
                                                <td style="color: var(--color-2) !important;">
                                                    '.strtoupper($products." DATACARD ".str_replace(["_","-"]," ",$product_smart_details["val_1"])).'
                                                ...
                                                <td>
                                                    <input style="text-align: center;" id="'.strtolower(trim($products)).'_datacard_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_smart_level" name="smart-price[]" type="text" value="'.$product_smart_details["val_2"].'" placeholder="Price" pattern="[0-9.]{1,}" title="Amount Must Be A Digit" class="input-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-100 s-width-100 m-padding-tp-10 s-padding-tp-5 m-padding-bm-10 s-padding-bm-5 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1" required/>
                                                </td>
                                                <td>
                                                    <input style="text-align: center;" id="'.strtolower(trim($products)).'_datacard_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_agent_level" name="agent-price[]" type="text" value="'.$product_agent_details["val_2"].'" placeholder="Price" pattern="[0-9.]{1,}" title="Amount Must Be A Digit" class="input-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-100 s-width-100 m-padding-tp-10 s-padding-tp-5 m-padding-bm-10 s-padding-bm-5 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1" required/>
                                                </td>
                                                <td>
                                                    <input style="text-align: center;" id="'.strtolower(trim($products)).'_datacard_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_api_level" name="api-price[]" type="text" value="'.$product_api_details["val_2"].'" placeholder="Price" pattern="[0-9.]{1,}" title="Amount Must Be A Digit" class="input-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-100 s-width-100 m-padding-tp-10 s-padding-tp-5 m-padding-bm-10 s-padding-bm-5 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1" required/>
                                                </td>
                                                <td>
                                                    <input style="text-align: center;" id="'.strtolower(trim($products)).'_datacard_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_days" name="product-days[]" type="text" value="'.$product_api_details["val_3"].'" placeholder="Days" pattern="[0-9.]{1,}" title="Days Must Be A Digit" class="input-box outline-none color-4 bg-10 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-100 s-width-100 m-padding-tp-10 s-padding-tp-5 m-padding-bm-10 s-padding-bm-5 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1" required/>
                                                </td>
                                            </tr>'; 
                    ...
                    <button id="" name="update-price" type="submit" style="user-select: auto;" class="button-box a-cursor outline-none color-2 bg-7 m-inline-block-dp s-inline-block-dp outline-none onhover-bg-color-5 br-radius-5px br-width-4 br-color-4 m-width-74 s-width-74 m-float-rt m-clr-float-both s-float-rt s-clr-float-both m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-rt-2 s-margin-rt-2 m-margin-bm-1 s-margin-bm-1" >
                        UPDATE PRICE
                    </button><br>
                </form>
            </div><br/>
			...
			<div style="border: 1px solid var(--color-4); user-select: auto; cursor: grab;" class="bg-3 m-inline-block-dp s-inline-block-dp m-width-90 s-width-47 m-height-auto s-height-auto m-scroll-x s-scroll-x m-margin-lt-0 s-margin-lt-0">
				<form method="post" enctype="multipart/form-data" action="">
					...
					<button onclick="getCSVDetails('5');" type="button" style="user-select: auto;" class="button-box a-cursor outline-none color-2 bg-7 m-inline-block-dp s-inline-block-dp outline-none onhover-bg-color-5 br-radius-5px br-width-4 br-color-4 m-width-97 s-width-97 m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" >
						PROCESS
					</button>
				</form>
			</div><br/>
			
			<a onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "datacard.csv");' style="text-decoration: underline; user-select: auto;" class="a-cursor color-2 outline-none text-bold-600">Download Price CSV</a><br/>
			
            <span style="user-select: auto;" class="color-4 text-bold-600 m-font-size-18 s-font-size-20 m-inline-block-dp s-inline-block-dp m-margin-tp-2 s-margin-tp-2 m-margin-bm-1 s-margin-bm-1">DATACARD UPLOAD</span><br>
            <form method="post" action="">
                <select style="text-align: center;" id="admin-cards-isp" name="isp" onchange="adminCardsSwitch(); adminCardsSwitchReset();" class="select-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-90 s-width-47 m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" required/>
                    <?php
                        //All DATACARD PRODUCT
                        ...
                        if(mysqli_num_rows($get_datacard_products) >= 1){
                            ...
                            while($product_details = mysqli_fetch_assoc($get_datacard_products)){
                                ...
                                echo '<option value="'.strtolower($product_details["product_name"]).'">'.strtoupper($product_details["product_name"]).' '.$product_status.'</option>';
                            }
                        }else{
                ...
                <select style="text-align: center;" id="admin-cards-qty" name="qty" onchange="adminCardsSwitch();" class="select-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-90 s-width-47 m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" required/>
                    <option value="" default hidden selected>Choose Qty</option>
                    <?php
                        function datacardQty(){
                            ...
                            //All DATACARD PRODUCT
                            $get_datacard_products = mysqli_query($connection_server, "SELECT * FROM sas_cards WHERE vendor_id='".$get_logged_admin_details["id"]."'");
                            foreach($item_name_array_3 as $products){
                                ...
                                if($count_product_variety >= 1){
                                    foreach($product_variety as $product_qty){
                                        $return_product_option .= '<option value="'.$product_qty.'" product-isp="'.strtolower($products).'" hidden>'.$product_qty.'</option>';
                                    }
                                }
                            }
                ...
                <input style="text-align: center;" id="admin-cards-input" name="dial-code" onkeyup="" type="text" value="" placeholder="Recharge Code, Balance Code (seperated by commas)" class="input-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-87 s-width-45 m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" required/><br/>
                <div style="text-align: center;" class="color-4 bg-3 m-inline-block-dp s-inline-block-dp m-font-size-14 s-font-size-16 m-width-60 s-width-45">
                    <span id="user-status-span" class="a-cursor" style="user-select: auto;">DATACARD LISTS</span>
                </div><br/>
                <textarea style="text-align: center; resize: none;" id="admin-cards-textarea" name="cards" onkeyup="" placeholder="Datacards seperated by commas" class="outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-87 s-width-45 m-height-20rem s-height-20rem m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" ></textarea><br>
                <?php
                    function datacardTextarea(){
                        global $connection_server, $get_logged_admin_details, $array_mtn_product_variety, $array_airtel_product_variety, $array_glo_product_variety, $array_9mobile_product_variety;
                        $item_name_array_3 = array("mtn", "airtel", "glo", "9mobile");
                        //All DATACARD PRODUCT
                        $get_datacard_products = mysqli_query($connection_server, "SELECT * FROM sas_cards WHERE vendor_id='".$get_logged_admin_details["id"]."'");
                        foreach($item_name_array_3 as $products){
                            ...
                            if($count_product_variety >= 1){
                                foreach($product_variety as $product_qty){
                                    ...
                                    $get_datacard_product_cards = mysqli_fetch_array(mysqli_query($connection_server, "SELECT * FROM sas_cards WHERE vendor_id='".$get_logged_admin_details["id"]."' && card_name='".$textarea_id_name."' LIMIT 1"));
                                    $return_product_textarea .= 
                                    '<textarea style="text-align: center; resize: none;" id="'.$textarea_id_name.'" name="" onkeyup="" placeholder="Datacards seperated by commas" class="m-none-dp s-none-dp" >'.$get_datacard_product_cards["cards"].'</textarea>
                                    <input style="text-align: center; resize: none;" id="'.$input_id_name.'" name="" onkeyup="" placeholder="Dial Code" class="m-none-dp s-none-dp" value="'.$get_datacard_product_cards["dial_code"].'" />';
                                }
                            }
                        }
