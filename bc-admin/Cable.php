<?php session_start();
    ...
    if(isset($_POST["install-product"])){
        ...
        $array_startimes_product_variety = array("nova_weekly", "basic_weekly", "smart_weekly", "classic_weekly", "super_weekly", "nova", "basic", "smart", "classic", "super", "chinese_dish", "nova_antenna");
        $array_dstv_product_variety = array("padi","yanga","confam","compact","premium","padi_extraview","yanga_extraview","confam_extraview","compact_extra_view","compact_plus","compact_plus_extra_view","compact_plus_frenchplus_extra_view","premium_extra_view","premium_french_extra_view");
        $array_gotv_product_variety = array("smallie", "jinja", "jolli", "max", "super");
            ...
            if(!empty($product_name)){
                if(in_array($product_name, $products_array)){
					if(is_numeric($item_status) && in_array($item_status, array("0", "1"))){
                        $select_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='$api_id' && api_type='cable'");
                        $select_cable_status_lists = mysqli_query($connection_server, "SELECT * FROM sas_cable_status WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
						if(mysqli_num_rows($select_api_lists) == 1){    
                            if(mysqli_num_rows($select_cable_status_lists) == 0){
                            	mysqli_query($connection_server, "INSERT INTO sas_cable_status (vendor_id, api_id, product_name, status) VALUES ('".$get_logged_admin_details["id"]."', '$api_id', '$product_name', '$item_status')");
                            }else{
                            	mysqli_query($connection_server, "UPDATE sas_cable_status SET api_id='$api_id', status='$item_status' WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
                            }
                           	...
                           	foreach($account_level_table_name_arrays as $account_level_table_name){	
                           		...
                           		if(mysqli_num_rows($select_product_details) == 1){
                                    ...
                                    $select_all_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_type='cable'");
                        ...
                        //Invalid Cable Status
                        $json_response_array = array("desc" => "Invalid Cable Status");
                		$json_response_encode = json_encode($json_response_array,true);
                	}
                }else{
...
<body>
	<?php include("../func/bc-admin-header.php"); ?>	
        <div style="text-align: center;" class="bg-10 m-block-dp s-block-dp m-position-rel s-position-rel br-radius-5px m-width-94 s-width-94 m-height-auto s-height-auto m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-padding-tp-5 s-padding-tp-3 m-padding-bm-5 s-padding-bm-3 m-margin-lt-2 s-margin-lt-2 m-margin-bm-2 s-margin-bm-2">
            <span style="user-select: auto;" class="text-bg-1 color-4 text-bold-500 m-font-size-25 s-font-size-30 m-inline-block-dp s-inline-block-dp m-margin-bm-1 s-margin-bm-1">CABLE SETTINGS</span><br>
            ...
            <div style="text-align: center; user-select: auto;" class="bg-3 m-inline-block-dp s-inline-block-dp m-width-70 s-width-50 m-margin-tp-1 s-margin-tp-1 m-margin-bm-0 s-margin-bm-0">
                <img alt="Startimes" id="startimes-lg" product-name-array="startimes,dstv,gotv" src="/asset/startimes.jpg" onclick="tickProduct(this, 'startimes', 'api-product-name', 'install-product', 'jpg');" class="a-cursor bg-2 br-radius-100px m-inline-block-dp s-inline-block-dp onhover-img-filter m-width-20 s-width-22 m-margin-tp-1 s-margin-tp-1 m-margin-bm-1 s-margin-bm-1 "/>
                <img alt="Dstv" id="dstv-lg" product-name-array="startimes,dstv,gotv" src="/asset/dstv.jpg" onclick="tickProduct(this, 'dstv', 'api-product-name', 'install-product', 'jpg');" class="a-cursor bg-2 br-radius-100px m-inline-block-dp s-inline-block-dp onhover-img-filter m-width-20 s-width-22 m-margin-tp-1 s-margin-tp-1 m-margin-bm-1 s-margin-bm-1 m-margin-lt-1 s-margin-lt-1"/>
                <img alt="Gotv" id="gotv-lg" product-name-array="startimes,dstv,gotv" src="/asset/gotv.jpg" onclick="tickProduct(this, 'gotv', 'api-product-name', 'install-product', 'jpg');" class="a-cursor bg-2 br-radius-100px m-inline-block-dp s-inline-block-dp onhover-img-filter m-width-20 s-width-22 m-margin-tp-1 s-margin-tp-1 m-margin-bm-1 s-margin-bm-1 m-margin-lt-1 s-margin-lt-1"/>
            </div><br/>
            <form method="post" action="">
                ...
                <div style="text-align: center;" class="color-4 bg-3 m-inline-block-dp s-inline-block-dp m-font-size-14 s-font-size-16 m-width-60 s-width-45">
                    <span id="user-status-span" class="a-cursor" style="user-select: auto;">CABLE STATUS</span>
                </div><br/>
            ...
            <div style="border: 1px solid var(--color-4); user-select: auto; cursor: grab;" class="bg-3 m-inline-block-dp s-inline-block-dp m-width-90 s-width-47 m-height-auto s-height-auto m-scroll-x s-scroll-x m-margin-lt-0 s-margin-lt-0">
                <table style="width: 100% !important;" class="table-tag m-font-size-12 s-font-size-14" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                    ...
                    <?php
                        $item_name_array = array("startimes", "dstv", "gotv");
            ...
            <span style="user-select: auto;" class="color-4 text-bold-600 m-font-size-18 s-font-size-20 m-inline-block-dp s-inline-block-dp m-margin-tp-2 s-margin-tp-2 m-margin-bm-1 s-margin-bm-1">CABLE DISCOUNT</span><br>
            <div style="border: 1px solid var(--color-4); user-select: auto; cursor: grab;" class="bg-3 m-inline-block-dp s-inline-block-dp m-width-90 s-width-47 m-height-auto s-height-auto m-scroll-x s-scroll-x m-margin-lt-0 s-margin-lt-0">
                <form method="post" action="">
                    <table style="width: 100% !important;" class="table-tag m-font-size-12 s-font-size-14" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                        <tr>
                        ...
                        <?php
                            $item_name_array_2 = array("startimes", "dstv", "gotv");
                                    ...
                                    while(($product_smart_details = mysqli_fetch_assoc($product_smart_table)) && ($product_agent_details = mysqli_fetch_assoc($product_agent_table)) && ($product_api_details = mysqli_fetch_assoc($product_api_table))){
                                        echo 
                                            '<tr style="background-color: transparent !important;">
                                                ...
                                                <td>
                                                    <input style="text-align: center;" name="api-price[]" id="'.strtolower(trim($products)).'_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_api_level" type="text" value="'.$product_api_details["val_2"].'" placeholder="Price" pattern="[0-9.]{1,}" title="Amount Must Be A Digit" class="input-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-100 s-width-100 m-padding-tp-10 s-padding-tp-5 m-padding-bm-10 s-padding-bm-5 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1" required/>
                                                </td>
                                            </tr>';	
                    ...
                    <button id="" name="update-price" type="submit" style="user-select: auto;" class="button-box a-cursor outline-none color-2 bg-7 m-inline-block-dp s-inline-block-dp outline-none onhover-bg-color-5 br-radius-5px br-width-4 br-color-4 m-width-67 s-width-62 m-float-rt m-clr-float-both s-float-rt s-clr-float-both m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-rt-2 s-margin-rt-2 m-margin-bm-1 s-margin-bm-1" >
                        UPDATE PRICE
                    </button><br>
                </form>
            </div><br/>
            ...
            <a onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "cable.csv");' style="text-decoration: underline; user-select: auto;" class="a-cursor color-2 outline-none text-bold-600">Download Price CSV</a>
