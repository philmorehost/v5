<?php session_start();
    ...
    if(isset($_POST["install-product"])){
        ...
        $array_mtn_product_variety = array("75mb_1day","110mb_1day","230mb_1day" ,"500mb","750mb_Free_1hour_TT/YT/IG_3days","1gb_1.5mins_1day","1.5gb","2.5gb_1day","2.5gb_2days","1gb_7days","2gb_2days","1.5gb_7days","3.2gb_2days","6gb_7days","11gb_7days","7gb_30days","10gb_10mins+2gb_YouTube_30days","12.5gb_30days","16.5gb_10Minutes_30days","40gb_60days","36gb_30days","75gb_30days","90gb_60days","165gb_30days","150gb_60days","200gb_30days","200gb_60days","250gb_30days","200gb_30days","480gb_90days");
        $array_airtel_product_variety = array("150mb_1day","300mb_2days","600mb_2days","1gb_(Social)3days","1.5gb_2days","2gb_2days","3gb_7days","5gb_2days","7gb_7days","10gb_30days");
        $array_glo_product_variety = array("750mb_1day","1.5gb_1day","2.5gb_2days","10gb_7days");
            ...
            if(!empty($product_name)){
                if(in_array($product_name, $products_array)){
                    if(is_numeric($item_status) && in_array($item_status, array("0", "1"))){
                        $select_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='$api_id' && api_type='shared-data'");
                        $select_shared_data_status_lists = mysqli_query($connection_server, "SELECT * FROM sas_shared_data_status WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
                        if(mysqli_num_rows($select_api_lists) == 1){    
                            if(mysqli_num_rows($select_shared_data_status_lists) == 0){
                                mysqli_query($connection_server, "INSERT INTO sas_shared_data_status (vendor_id, api_id, product_name, status) VALUES ('".$get_logged_admin_details["id"]."', '$api_id', '$product_name', '$item_status')");
                            }else{
                                mysqli_query($connection_server, "UPDATE sas_shared_data_status SET api_id='$api_id', status='$item_status' WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
                            }
                            ...
                            foreach($account_level_table_name_arrays as $account_level_table_name){ 
                                ...
                                if(mysqli_num_rows($select_product_details) == 1){
                                    ...
                                    $select_all_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_type='shared-data'");
                        ...
                        //Invalid SHARED DATA Status
                        $json_response_array = array("desc" => "Invalid shared-data Status");
                        $json_response_encode = json_encode($json_response_array,true);
                    }
                }else{
...
<body>
    <?php include("../func/bc-admin-header.php"); ?>    
        <div style="text-align: center;" class="bg-10 m-block-dp s-block-dp m-position-rel s-position-rel br-radius-5px m-width-94 s-width-94 m-height-auto s-height-auto m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-padding-tp-5 s-padding-tp-3 m-padding-bm-5 s-padding-bm-3 m-margin-lt-2 s-margin-lt-2 m-margin-bm-2 s-margin-bm-2">
            <span style="user-select: auto;" class="text-bg-1 color-4 text-bold-500 m-font-size-25 s-font-size-30 m-inline-block-dp s-inline-block-dp m-margin-bm-1 s-margin-bm-1">SHARED DATA SETTINGS</span><br>
            ...
            <form method="post" action="">
                ...
                <div style="text-align: center;" class="color-4 bg-3 m-inline-block-dp s-inline-block-dp m-font-size-14 s-font-size-16 m-width-60 s-width-45">
                    <span id="user-status-span" class="a-cursor" style="user-select: auto;">SHARED DATA STATUS</span>
                </div><br/>
            ...
            <span style="user-select: auto;" class="color-4 text-bold-600 m-font-size-18 s-font-size-20 m-inline-block-dp s-inline-block-dp m-margin-tp-2 s-margin-tp-2 m-margin-bm-1 s-margin-bm-1">INSTALLED SHARED DATA STATUS</span><br>
            ...
            <span style="user-select: auto;" class="color-4 text-bold-600 m-font-size-18 s-font-size-20 m-inline-block-dp s-inline-block-dp m-margin-tp-2 s-margin-tp-2 m-margin-bm-1 s-margin-bm-1">SHARED DATA DISCOUNT</span><br>
            <div style="border: 1px solid var(--color-4); user-select: auto; cursor: grab;" class="bg-3 m-inline-block-dp s-inline-block-dp m-width-90 s-width-47 m-height-auto s-height-auto m-scroll-x s-scroll-x m-margin-lt-0 s-margin-lt-0">
                <form method="post" action="">
                    <table style="width: 100% !important;" class="table-tag m-font-size-12 s-font-size-14" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                        <tr>
            ...
            <a onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "shared-data.csv");' style="text-decoration: underline; user-select: auto;" class="a-cursor color-2 outline-none text-bold-600">Download Price CSV</a>
