<?php session_start();
        ...
        if(!empty($api_id) && is_numeric($api_id)){
            if(!empty($apikey)){
            	if(is_numeric($apistatus) && in_array($apistatus, array("0", "1"))){
            		$select_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='$api_id' && api_type='bulk-sms'");
                	if(mysqli_num_rows($select_api_lists) == 1){
                    	mysqli_query($connection_server, "UPDATE sas_apis SET api_key='$apikey', status='$apistatus' WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='$api_id' && api_type='bulk-sms'");
    ...
    if(isset($_POST["install-product"])){
        ...
        $array_mtn_product_variety = array("standard_sms");
        $array_airtel_product_variety = array("standard_sms");
        $array_glo_product_variety = array("standard_sms");
        $array_9mobile_product_variety = array("standard_sms");
            ...
            if(!empty($product_name)){
                if(in_array($product_name, $products_array)){
					if(is_numeric($item_status) && in_array($item_status, array("0", "1"))){
                        $select_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='$api_id' && api_type='bulk-sms'");
                        $select_bulk_sms_status_lists = mysqli_query($connection_server, "SELECT * FROM sas_bulk_sms_status WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
						if(mysqli_num_rows($select_api_lists) == 1){    
                            if(mysqli_num_rows($select_bulk_sms_status_lists) == 0){
                            	mysqli_query($connection_server, "INSERT INTO sas_bulk_sms_status (vendor_id, api_id, product_name, status) VALUES ('".$get_logged_admin_details["id"]."', '$api_id', '$product_name', '$item_status')");
                            }else{
                            	mysqli_query($connection_server, "UPDATE sas_bulk_sms_status SET api_id='$api_id', status='$item_status' WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$product_name'");
                            }
                           	...
                           	foreach($account_level_table_name_arrays as $account_level_table_name){	
                           		...
                           		if(mysqli_num_rows($select_product_details) == 1){
                                    ...
                                    $select_all_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_type='bulk-sms'");
                        ...
                        //Invalid bulk-sms Status
                        $json_response_array = array("desc" => "Invalid bulk-sms Status");
                		$json_response_encode = json_encode($json_response_array,true);
                	}
                }else{
...
<!DOCTYPE html>
<head>
    <title>Bulk SMS API | <?php echo $get_all_super_admin_site_details["site_title"]; ?></title>
    <meta charset="UTF-8" />
...
<body>
	<?php include("../func/bc-admin-header.php"); ?>	
        <div style="text-align: center;" class="bg-10 m-block-dp s-block-dp m-position-rel s-position-rel br-radius-5px m-width-94 s-width-94 m-height-auto s-height-auto m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-padding-tp-5 s-padding-tp-3 m-padding-bm-5 s-padding-bm-3 m-margin-lt-2 s-margin-lt-2 m-margin-bm-2 s-margin-bm-2">
            <span style="user-select: auto;" class="text-bg-1 color-4 text-bold-500 m-font-size-25 s-font-size-30 m-inline-block-dp s-inline-block-dp m-margin-bm-1 s-margin-bm-1">BULK SMS SETTINGS</span><br>
            ...
            <form method="post" action="">
                <select style="text-align: center;" id="" name="api-id" onchange="getWebApikey(this);" class="select-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-63 s-width-47 m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" required/>
                    <?php
                        //All bulk-sms API
                        $get_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_type='bulk-sms'");
            ...
            <form method="post" action="">
                ...
                <div style="text-align: center;" class="color-4 bg-3 m-inline-block-dp s-inline-block-dp m-font-size-14 s-font-size-16 m-width-60 s-width-45">
                    <span id="user-status-span" class="a-cursor" style="user-select: auto;">BULK SMS STATUS</span>
                </div><br/>
                <select style="text-align: center;" id="" name="item-status" onchange="" class="select-box outline-none color-4 bg-2 m-inline-block-dp s-inline-block-dp outline-none br-radius-5px br-width-4 br-color-4 m-width-63 s-width-47 m-padding-tp-2 s-padding-tp-1 m-padding-bm-2 s-padding-bm-1 m-padding-lt-1 s-padding-lt-1 m-padding-rt-1 s-padding-rt-1 m-margin-bm-1 s-margin-bm-1" required/>
                    <option value="" default hidden selected>Choose Bulk SMS Status</option>
                    <option value="1" >Enabled</option>
                    <option value="0" >Disabled</option>
                </select><br/>
            ...
            <span style="user-select: auto;" class="color-4 text-bold-600 m-font-size-18 s-font-size-20 m-inline-block-dp s-inline-block-dp m-margin-tp-2 s-margin-tp-2 m-margin-bm-1 s-margin-bm-1">INSTALLED BULK SMS STATUS</span><br>
            <div style="border: 1px solid var(--color-4); user-select: auto; cursor: grab;" class="bg-3 m-inline-block-dp s-inline-block-dp m-width-90 s-width-47 m-height-auto s-height-auto m-scroll-x s-scroll-x m-margin-lt-0 s-margin-lt-0">
                <table style="width: 100% !important;" class="table-tag m-font-size-12 s-font-size-14" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                    ...
                    <?php
                        ...
                        $select_item_lists = mysqli_query($connection_server, "SELECT * FROM sas_bulk_sms_status WHERE vendor_id='".$get_logged_admin_details["id"]."' && $items_statement");
                        if(mysqli_num_rows($select_item_lists) >= 1){
                            while($list_details = mysqli_fetch_assoc($select_item_lists)){
                                $select_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='".$list_details["api_id"]."' && api_type='bulk-sms'");
                                if(mysqli_num_rows($select_api_lists) == 1){
                                    ...
                                    if(mysqli_num_rows($select_api_lists) == 0){
                                        $api_route_web = mysqli_num_rows($select_item_lists)."Invalid API Website";
                                    }else{
            ...
            <span style="user-select: auto;" class="color-4 text-bold-600 m-font-size-18 s-font-size-20 m-inline-block-dp s-inline-block-dp m-margin-tp-2 s-margin-tp-2 m-margin-bm-1 s-margin-bm-1">BULK SMS DISCOUNT</span><br>
            <div style="border: 1px solid var(--color-4); user-select: auto; cursor: grab;" class="bg-3 m-inline-block-dp s-inline-block-dp m-width-90 s-width-47 m-height-auto s-height-auto m-scroll-x s-scroll-x m-margin-lt-0 s-margin-lt-0">
                <form method="post" action="">
                    <table style="width: 100% !important;" class="table-tag m-font-size-12 s-font-size-14" title="Horizontal Scroll: Shift + Mouse Scroll Button">
                        <tr>
                        ...
                        <?php
                            ...
                            foreach($item_name_array_2 as $products){
                                $get_item_status_details = mysqli_fetch_array(mysqli_query($connection_server, "SELECT * FROM sas_bulk_sms_status WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_name='$products'"));
                                $get_api_lists = mysqli_query($connection_server, "SELECT * FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' && id='".$get_item_status_details["api_id"]."' && api_type='bulk-sms'");
            ...
            <a onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "bulk-sms.csv");' style="text-decoration: underline; user-select: auto;" class="a-cursor color-2 outline-none text-bold-600">Download Price CSV</a>
